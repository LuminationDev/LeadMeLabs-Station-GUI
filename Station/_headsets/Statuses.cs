...
public class Statuses
{
    ...
    private void SendReadyMessage()
    {
        ...
        {
            ScheduledTaskQueue.EnqueueTask(() => SessionController.UpdateSoftwareState("Ready to go"), TimeSpan.FromSeconds(0));
        }
    }
    ...
    private void SendLostMessage(bool oldConnectionStatus, DeviceStatus newConnectionStatus)
    {
        ...
        ScheduledTaskQueue.EnqueueTask(
            () => SessionController.UpdateSoftwareState($"{message}"),
    ...
    public void QueryStatuses()
    {
        ...
        vrStatuses.Add("Headsets", headset);

        // each controller
        JArray controllersJArray = new JArray();
        foreach (var vrController in Controllers)
        {
            JObject controller = new()
            {
                { "role", vrController.Value.Role.ToString()},
        ...
        JObject baseStationDetails = new()
        {
            { "connectedBaseStations", active },
            { "totalBaseStations", baseStations.Count },
        };
        vrStatuses.Add("BaseStations", baseStationDetails);
        
        JObject deviceQuery = new() { { "DeviceQuery", vrStatuses } };
        Manager.SendMessage("NUC", "Station", deviceQuery);
    }
