...
public class ConsoleViewModel : ObservableObject
{
    ...
    /// Used for binding the MainWindow Mock Console
    /// </summary>
    private MockConsole.LogLevel _currentLogLevel = MockConsole.LogLevel.Normal;

    public MockConsole.LogLevel CurrentLogLevel
    {
    ...
    {
        if (CurrentLogLevel == MockConsole.LogLevel.Verbose)
        {
            CurrentLogLevel = MockConsole.LogLevel.Off;
        }
        else
        {
            CurrentLogLevel++;
    ...
    {
        ConsoleText = "";
        WriteLine("Cleared", MockConsole.LogLevel.Error);
    }
    ...
    public void WriteLine(string message, MockConsole.LogLevel level)
    {
        ...
        if (level > CurrentLogLevel || CurrentLogLevel == MockConsole.LogLevel.Off) return;
