...
public class Statuses
{
    public string HeadsetDescription { private set; get; } = "Unknown";
         ...
         public static readonly Dictionary<string, VrBaseStation> baseStations = new();
                 ...
                 if (duplicate)
                 {
                     foreach (var vrController in Controllers)
                     {
                         if (vrController.Value.Role == role)
                         {
                             ...
                             MockConsole.WriteLine($"Duplicate controller - Role: {Enum.GetName(typeof(DeviceRole), role)}. Resetting dictionary.", MockConsole.LogLevel.Normal);
                         }
                     }
                     Controllers.Clear();
                     return;
