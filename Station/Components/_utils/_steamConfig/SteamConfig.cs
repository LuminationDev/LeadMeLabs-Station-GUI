...
public static class SteamConfig
{
    ...
    private static bool steamStatsReported;
    ...
    private static async void ReadAndReportSteamStats()
    {
        ...
        try
        {
            _ = Task.Run(async () =>
            {
                ...
                await using (var fs = new FileStream(fileLocation, FileMode.Open, FileAccess.Read, FileShare.ReadWrite,
                ...
                if (experienceStats.Count > 0)
                {
                    ...
                    {
                        strJSON += String.Format("\"{0}/{1}\": {2},", experienceStat.Key, datetime,
